function drift = calcDrift(Sm, S, maxDrift, interpMethod)
%compute the drift error from sinogram matching
% Input: 
%   Sm:  the measured Sinogram with beamline drifted, NTheta*NTau 2D matrix, assume it is interploation from S here. 
...
switch lower(interpMethod)
    case lower('linearOld')  % old a, b notations before 2019.07.11, only good for interger maxDrift
        for i = maxDriftCeil+1 : NTau-maxDriftCeil % compute d(i), note zero boundaries d(i)=0 for i < maxDriftCeil+1 or i > NTau-maxDriftCeil
            if norm(Sm(:,i), 2) < NTheta*eps && norm(S(:,i), 2) < NTheta*eps; continue;  end  % empty beam matched to empty beam, skip & set drift to 0
            objOpt = inf;
