...
if ~batchMode
    gaussianSTD = 0; % 0.01; % [0, 0.01, 0.05]
end
for idxScan = 1: NScan
...
maxIterA = 400; % 100 is not enough
regWt = 1e-6; % 1.3e05 or 1e-5 similar or 1e-8*max(WGT(:))/LNormalizer^2;  1e-6 to 1e-8 both good for phantom, use 1e-8 for brain, especically WGT is scaled to maximum of 1 not 40
maxSVDofA = 1e-2; % for TV, maxSVDofA = 1e-5/1e-4 or 1e-6/LNormalizer^2 NOT maxSVDofA = 1e-6/LNormalizer,  make sure it is actually much smaller than svds(L, 1), so that first step in TwIST is long enough 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
S_nodrift =SAll(:,:,1); Sm = SAll(:,:,2);  driftGT = driftGTAll{2}; 
...
elseif driftType == 3
    [WRecs, SRecs, drift, driftAll,info] = recTompographyWithDrift_TypeIII(S_nodrift, Sm, paraTwist, WSz, maxDrift, driftGT,'new',normeps);
    [WRecs_old, SRecs_old, drift_old, driftAll_old,info_old] = recTompographyWithDrift_TypeIII(S_nodrift,Sm, paraTwist, WSz, maxDrift, driftGT,'old',normeps);
elseif driftType == 4
...
% figure, plot(1:length(info.misfit_nodrift_history),info.misfit_nodrift_history,'linewidth',1.5)
% hold on 
% plot(1:length(info.misfit_nodrift_history),info_old.misfit_nodrift_history,'linewidth',1.5)
% xlim([1,10])
% legend('new','old')
% title('misfit_nodrift term history')
% set(gca,'fontsize',16)  
