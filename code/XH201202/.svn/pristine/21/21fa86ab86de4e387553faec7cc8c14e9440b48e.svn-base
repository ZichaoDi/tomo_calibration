...
% Modified by Xiang Huang 2016.05.11,  2017.01.29
...
%matlabMonitorSize = monitorPositions(:,3:4) - monitorPositions(:,1:2) + 1; % Original Code
matlabMonitorSize = monitorPositions(:,3:4); % Xiang modified@2016.05.11, monitorPositions = [[x1 y1 width1 height1; x2 y2 width2 height2] for dual monitor


if ~exist('monitor','var') || isempty(monitor)
    %No monitor specified, use the last monitor such as 2nd/3rd monitor Xiang modified@2016.05.11. (original code use the primary as default)
    %monitor = 1;
    monitor = size(monitorPositions, 1); % the last monitor, monitor = 1 if there is only 1 monitor, monitor = 2 if dual monitor    
end
...
if ~exist('handles','var') || isempty(handles)
    ...
    if isempty(handles) % 2017.01.29 added
        warning('Currently has no figures!');
        return
    end
    %handles = handles(end:-1:1); % Re-order so that first created is in upper left % Original Code.   
    [~, idxMap] = sort([handles.Number]);  handles = handles(idxMap);  % Re-order so that the smallest Figure No is in upper left, Xiang modified@2016.05.11,
end
nFigures = numel(handles); % Xiang Changed from length() to numel()

% Put all of the figures in pixel units
units = {handles.Units}; % save current units and will restore in the end. old code units = get(handles, 'units') will result error for the call units{ii} in the end as units won't be cell when there is only one figure 
...
if resize
    ...
    for row = 1:nRows
        for col = 1:nCols
            ...
            figure(handles(pnum))
            %2017.01.29: Ubuntu has bug: can't change the size of a originally full size figure (even if using one monitor & one workspace).
