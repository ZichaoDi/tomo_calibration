...
%L = LAll{1}; Lnormalizer = full(max(sum(L,2))); L= L/Lnormalizer;
%%
%driftGT = driftGTAll{2}; save('20190711_brain.mat', 'SAll', 'driftGT', 'maxDrift', 'WGT', 'LAll', 'gaussianSTD')
driftGT = driftGTAll{2}; save('20190711_phantom.mat', 'SAll', 'driftGT', 'maxDrift', 'WGT', 'LAll', 'gaussianSTD')
...
figure(figNo+3); imagesc(SAll(:,:,2)); axis image; title(sprintf('Sinogram With drift, PSNR=%.2fdB compare without drift', difference(SAll(:,:,2), SAll(:,:,1))));
tilefigs;
%%
...
recTompographyWithDrift_ICIP_20191709
