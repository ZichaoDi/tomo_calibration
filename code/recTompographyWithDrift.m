...
W2 = solveTwist(SAll(:,:,2), LExactfromDrift, paraTwist{:}); figure(figNo+115); W = W2;  imagesc(W); axis image; title(sprintf('Rec twist %s, PSNR=%.2fdB, %s, regWt=%.1e, maxIter=%d', 'Use Exact Forward Model from Drift', difference(W, WGT), regType, regWt, maxIterA));
...
W2 = solveTwist(SAll(:,:,2), LExactfromDrift, paraTwist2{:}); figure(figNo+116); W = W2;  imagesc(W); axis image; title(sprintf('Rec twist %s, PSNR=%.2fdB, %s, regWt=%.1e, maxIter=%d', 'Use Exact Forward Model from Drift', difference(W, WGT), regType, paraTwist2{6}, maxIterA));
