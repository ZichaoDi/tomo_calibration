...
if strcmp(reg,'opt') && parameters.cycle == 1
    lambda = 1;
elseif strcmp(reg,'opt') && parameters.cycle > 1
    lambda = parameters.lambda1;
elseif strcmp(reg,'discrep')
    lambda = 1;
    ...
    if H(k+1,k)<1e-10
        ...
        gmres_res = abs((UH(:,k+1)'*d))/nb;
            ...
            if strcmp(reg,'opt')
                ...
                xexhat = UX'*(xexhat*VX);
                xexhat = xexhat(:)./precX;
%                 xexhat = reshape(xexhat, nim, nim);
%                 xexhat = UX*(xexhat*VX');
                xhat = VH'*(Vmatr(:,1:k)'*xexhat(:));
%                 xhat = VH'*(Vmatr(:,1:k)'*xex);
                sH = SH;
                myfun = @(lambda) norm(((conj(sH)./(abs(sH).^2+lambda)).*c(1:end-1) - xhat));
                options.TolX = eps;
                lambda = fminbnd(myfun, 0, sH(1),options);
            end
