function [Xtot, Xfinals, Xbest, Enrm_tot, Rnrm_tot, cyclesIt, sXcycles, cyclesBest, Lambda_tot] = irn_lsqr_nnr(A, b, xex, x0, parameters)
...
for i = 1:cycles
    ...
    optnnr.cycle = i;
    optnnr.gamma = gamma;
    if i > 1 && strcmp(reg,'discrep')
        lambda1 = Lambda_nnr(end);
        optnnr.lambda1 = lambda1;
        optnnr.reg = lambda1;
    end
    [X_nnr, Enrm_nnr, Rnrm_nnr, Lambda_nnr] = lsqr_nnr(A, b, xex, x0, optnnr);
    ...
    cyclesBest(i) = indmin;
    cyclesIt(:,i) = ind;
    Xtot(:,totIt_temp+1:totIt_temp+ind) = X_nnr(:,1:ind);
