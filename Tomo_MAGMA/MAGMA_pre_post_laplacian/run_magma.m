...
L = delsq(numgrid('S',Nx+2));
%% 2. Algorithm Parameters
% These are based on the paper's experiments (Section 4.3)
params.lambda = 1e-10;   % Regularization parameter
params.kappa = 0.01;     % Coarse direction selection threshold (3.4)
...
params.T = 500;         % Max iterations for the main MAGMA loop
...
params.tolerance = 1e-8; % Convergence tolerance for ||D(x)||
params.fine_tolerance = 1e-8;
...
[L_level, R_level, P_level, N_vec, Lf_level] = setup_multilevel_operators_new_laplace(L, Nx, num_levels);
...
obj_f_fine  = @(x) (0.5/Nx^2) * norm(L_level{1} * x)^2;
obj_g_fine  = @(x) params.lambda * norm(x, 1);
grad_f_fine = @(x) (1/Nx^2)*L_level{1}' * (L_level{1} * x);
prox_op_fine = @prox_g;
...
max_iters = params.T;
obj_f_fine  = @(x) (0.5/Nx^2) * norm(L_level{1} * x)^2;
obj_g_fine  = @(x) (1/Nx^2)*params.lambda * norm(x, 1);
grad_f_fine = @(x) L_level{1}' * (L_level{1} * x);
prox_op_fine = @prox_g;
