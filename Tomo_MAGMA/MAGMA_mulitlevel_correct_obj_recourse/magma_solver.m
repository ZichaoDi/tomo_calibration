...
%MAGMA_SOLVER General MAGMA solver engine that tracks equivalent work.
    ...
    for k = 1:T
        ...
        % --- Decision Block ---
        % First, decide if a coarse correction should be attempted
        cond1 = norm(R_current * grad_F_mu) > kappa * norm(grad_F_mu);
        cond2 = (norm(x_k - x_tilde) > theta * norm(x_tilde)) || (q < Kd);
        attempt_coarse_step = cond1 && cond2 && level_no < length(params.L_level_full);
        
        s_k = 0; % Initialize s_k to ensure it's 0 if the coarse step isn't attempted
        ...
        % --- Main Logic: If coarse step was attempted and succeeded (s_k ~= 0), use it. ---
        % --- Otherwise (not attempted or s_k == 0), fall back to the proximal gradient step. ---
        if attempt_coarse_step && s_k ~= 0
            ...
            if is_fine_level
                ...
                history.x_k_coarse_hist{end+1} = x_k; 
                history.coarse_step_iters(end+1) = k;
            end
            q = 0; x_tilde = x_k;
            
            % Update eta and alpha based on s_k
            % if alpha_k == 0 
            ...
            work_this_iter = coarse_work + my_level_factor * 1;
        else
            % --- Gradient Step (Proximal or Standard) ---
            % This block executes if the coarse step was not attempted or if it failed (s_k=0)
            fprintf('    -> [Level %d] Perfroming prox grad step...\n', level_no);
            grad_at_xk = grad_f(x_k); % This might need recalculation if it was only in the 'if' block
        ...
        if is_fine_level && attempt_coarse_step && s_k ~= 0 % Store only if a coarse step was actually performed and succeeded
        ...
        if is_fine_level
            ...
            history.x_change(k) = norm(x_k - x_k_old) / numel(x_k);
